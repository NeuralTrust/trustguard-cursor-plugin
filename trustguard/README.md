...
Attribution sends the Cursor account email as `attributes.user.email`. The
collector already identifies the source. `consumer_id` is only sent when set
via `TRUSTGUARD_CONSUMER_ID` or `consumer_id` in config.
...
| `TRUSTGUARD_CONSUMER_ID` | `consumer_id` | _(unset)_ | Only source of `consumer_id`; omitted when unset. Cursor `user_email` goes to `attributes.user.email` |
