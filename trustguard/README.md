...
Attribution uses `consumer_id = cursor:<account-email>` from the hook
`user_email`. Override with `TRUSTGUARD_CONSUMER_ID` or `consumer_id` in config.
...
| `TRUSTGUARD_CONSUMER_ID` | `consumer_id` | `cursor:<os user>` | Override. Else runtime uses Cursor `user_email`. |
